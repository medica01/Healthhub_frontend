...
class _doc_bioState extends State<doc_bio> {
  ...
  Future<void> _validandsave()async{
    ...
    if(service.text.isEmpty || int.parse(service.text)<0 ||int.parse(service.text)>47){
    ...
    if(missingfield.isNotEmpty){
      showDialog(context: context, builder: (context)=>AlertDialog(
        title: Text("Missing field",style: TextStyle(color: Colors.red,fontWeight: FontWeight.bold,fontSize: 25),),
    ...
    try{
      ...
      if(response.statusCode==200 || response.statusCode == 201 || response.statusCode == 204){
        showDialog(context: context, builder: (context)=>AlertDialog(
          ...
          content: Text("Please check all the details or correct ?",style: TextStyle(fontWeight: FontWeight.bold),),
          actions: [
            Row(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        centerTitle: true,
        title: Text("sensational data only for developer",style: TextStyle(color: Color(0xff1f8acc)),),
      ),
      body: Center(
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              doc_form_field("enter qualification", qualification, TextInputType.text),
